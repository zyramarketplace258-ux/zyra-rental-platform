...
class OptionsUtil {
    ...
    _getKnownOptions(options, context) {
        const knownOptions = common3420f234_node.ObjectValue.empty();
        ...
        for (const knownOptionKey in this.optionDefinitions) {
            if (this.optionDefinitions.hasOwnProperty(knownOptionKey)) {
                ...
                if (knownOptionKey in options) {
                    ...
                    if (optionDefinition.nestedOptions && common3420f234_node.isPlainObject(optionValue)) {
                        ...
                        protoValue = common3420f234_node.parseData(optionValue, context) ?? undefined;
                    }
                    if (protoValue) {
                        knownOptions.set(common3420f234_node.FieldPath$1.fromServerFormat(optionDefinition.serverName), protoValue);
                    }
                }
            }
        }
    ...
    getOptionsProto(context, knownOptions, optionsOverride) {
        ...
        if (optionsOverride) {
            const optionsMap = new Map(common3420f234_node.mapToArray(optionsOverride, (value, key) => [
                common3420f234_node.FieldPath$1.fromServerFormat(key),
                value !== undefined ? common3420f234_node.parseData(value, context) : null
            ]));
...
function isIMapValue(obj) {
    ...
    if ('fields' in obj && (obj.fields === null || common3420f234_node.isPlainObject(obj.fields))) {
        return true;
    }
    return false;
}
...
function valueToDefaultExpr$1(value) {
    ...
    else if (common3420f234_node.isPlainObject(value)) {
        result = _map(value);
    }
...
function vectorToExpr$1(value) {
    ...
    else if (value instanceof common3420f234_node.VectorValue) {
        return constant(value);
    }
    else if (Array.isArray(value)) {
        return constant(common3420f234_node.vector(value));
    }
    else {
...
function fieldOrExpression$1(value) {
    if (common3420f234_node.isString(value)) {
...
class Expression {
    ...
    asBoolean() {
        if (this instanceof BooleanExpression) {
            ...
            throw new common3420f234_node.FirestoreError('invalid-argument', `Conversion of type ${typeof this} to BooleanExpression not supported.`);
        }
    }
    subtract(subtrahend) {
    ...
    timestampTruncate(granularity, timezone) {
        const internalGranularity = common3420f234_node.isString(granularity)
...
function _field(nameOrPath, methodName) {
    if (typeof nameOrPath === 'string') {
        if (common3420f234_node.DOCUMENT_KEY_NAME === nameOrPath) {
            return new Field(common3420f234_node.documentId()._internalPath, methodName);
        }
        return new Field(common3420f234_node.fieldPathFromArgument('field', nameOrPath), methodName);
    }
    else {
...
class Constant extends Expression {
     ...
     */
    _toProto(_) {
        common3420f234_node.hardAssert(this._protoValue !== undefined, 0x00ed);
    ...
    _readUserData(context) {
        ...
        if (isFirestoreValue(this._protoValue)) {
            ...
            this._protoValue = common3420f234_node.parseData(this.value, context);
        }
    }
}
...
class MapValue extends Expression {
    ...
    _toProto(serializer) {
        return common3420f234_node.toMapValue(serializer, this.plainObject);
    }
}
/**
 * @beta
...
class Ordering {
    ...
    _toProto(serializer) {
        return {
            mapValue: {
                fields: {
                    direction: common3420f234_node.toStringValue(this.direction),
...
function isSelectable(val) {
    const candidate = val;
    return (candidate.selectable && common3420f234_node.isString(candidate.alias) && isExpr(candidate.expr));
}
...
function isAliasedAggregate(val) {
    const candidate = val;
    return (common3420f234_node.isString(candidate.alias) &&
...
function toPipelineBooleanExpr(f) {
    if (f instanceof common3420f234_node.FieldFilter) {
        ...
        switch (f.op) {
            ...
            default:
                common3420f234_node.fail(0x9047);
        }
    }
    else if (f instanceof common3420f234_node.CompositeFilter) {
        switch (f.op) {
            ...
            default:
                common3420f234_node.fail(0x89ea);
        }
    }
...
function toPipeline(query, db) {
    let pipeline;
    if (common3420f234_node.isCollectionGroupQuery(query)) {
        pipeline = db.pipeline().collectionGroup(query.collectionGroup);
    }
    else if (common3420f234_node.isDocumentQuery(query)) {
        pipeline = db.pipeline().documents([common3420f234_node.doc(db, query.path.canonicalString())]);
    }
    else {
    ...
    // orders
    const orders = common3420f234_node.queryNormalizedOrderBy(query);
...
class AddFields extends Stage {
    ...
    _toProto(serializer) {
        return {
            ...
            args: [common3420f234_node.toMapValue(serializer, this.fields)]
        };
    }
    _readUserData(context) {
...
class Aggregate extends Stage {
    ...
    _toProto(serializer) {
        return {
            ...super._toProto(serializer),
            args: [
                common3420f234_node.toMapValue(serializer, this.accumulators),
                common3420f234_node.toMapValue(serializer, this.groups)
            ]
        };
    }
    _readUserData(context) {
...
class Distinct extends Stage {
    ...
    _toProto(serializer) {
        return {
            ...super._toProto(serializer),
            args: [common3420f234_node.toMapValue(serializer, this.groups)]
        };
    }
    _readUserData(context) {
        super._readUserData(context);
    ...
    _toProto(serializer) {
        return {
            ...
            args: [
                ...
                common3420f234_node.toStringValue(this.distanceMeasure)
            ]
        };
    }
    _readUserData(context) {
...
class Limit extends Stage {
    ...
    constructor(limit, options) {
        common3420f234_node.hardAssert(!isNaN(limit) && limit !== Infinity && limit !== -Infinity, 0x882c);
        super(options);
        this.limit = limit;
    }
    /**
    ...
    _toProto(serializer) {
        return {
            ...super._toProto(serializer),
            args: [common3420f234_node.toNumber(serializer, this.limit)]
        };
    }
}
/**
...
class Offset extends Stage {
    ...
    _toProto(serializer) {
        return {
            ...super._toProto(serializer),
            args: [common3420f234_node.toNumber(serializer, this.offset)]
        };
    }
}
/**
...
class Select extends Stage {
    ...
    _toProto(serializer) {
        return {
            ...super._toProto(serializer),
            args: [common3420f234_node.toMapValue(serializer, this.selections)]
        };
    }
    _readUserData(context) {
        super._readUserData(context);
...
class Sample extends Stage {
    ...
    _toProto(serializer) {
        return {
            ...super._toProto(serializer),
            args: [common3420f234_node.toNumber(serializer, this.rate), common3420f234_node.toStringValue(this.mode)]
        };
    }
    _readUserData(context) {
        super._readUserData(context);
...
class Union extends Stage {
    ...
    _toProto(serializer) {
        return {
            ...super._toProto(serializer),
            args: [common3420f234_node.toPipelineValue(this.other._toProto(serializer))]
        };
    }
    _readUserData(context) {
        super._readUserData(context);
...
class Replace extends Stage {
    ...
    _toProto(serializer) {
        return {
            ...super._toProto(serializer),
            args: [this.map._toProto(serializer), common3420f234_node.toStringValue(Replace.MODE)]
        };
    }
    _readUserData(context) {
        super._readUserData(context);
...
function readUserDataHelper(expressionMap, context) {
    if (common3420f234_node.isUserData(expressionMap)) {
...
class PipelineSource {
    ...
    collection(collectionOrOptions) {
        ...
        const options = common3420f234_node.isString(collectionOrOptions) ||
            common3420f234_node.isCollectionReference(collectionOrOptions)
            ? {}
            : collectionOrOptions;
        const collectionRefOrString = common3420f234_node.isString(collectionOrOptions) ||
            common3420f234_node.isCollectionReference(collectionOrOptions)
            ? collectionOrOptions
            : collectionOrOptions.collection;
        // Validate that a user provided reference is for the same Firestore DB
        if (common3420f234_node.isCollectionReference(collectionRefOrString)) {
            this._validateReference(collectionRefOrString);
        }
        // Convert user land convenience types to internal types
        const normalizedCollection = common3420f234_node.isString(collectionRefOrString)
    ...
    collectionGroup(collectionIdOrOptions) {
        ...
        let collectionId;
        let options;
        if (common3420f234_node.isString(collectionIdOrOptions)) {
    ...
    documents(docsOrOptions) {
        ...
        docs
            .filter(v => v instanceof common3420f234_node.DocumentReference)
            .forEach(dr => this._validateReference(dr));
        // Convert user land convenience types to internal types
        const normalizedDocs = docs.map(doc => common3420f234_node.isString(doc) ? doc : doc.path);
        // Create stage object
    ...
    _validateReference(reference) {
        ...
        if (!refDbId.isEqual(this.databaseId)) {
            throw new common3420f234_node.FirestoreError(common3420f234_node.Code.INVALID_ARGUMENT, `Invalid ${reference instanceof common3420f234_node.CollectionReference
                ? 'CollectionReference'
                : 'DocumentReference'}. ` +
...
class PipelineResult {
    ...
    get(fieldPath) {
        ...
        const value = this._fields.field(common3420f234_node.fieldPathFromArgument('DocumentSnapshot.get', fieldPath));
        if (value !== null) {
...
function selectablesToMap(selectables) {
    ...
    for (const selectable of selectables) {
        ...
        if (typeof selectable === 'string') {
            ...
            common3420f234_node.fail(0x5319, { selectable });
        }
        if (result.get(alias) !== undefined) {
            throw new common3420f234_node.FirestoreError('invalid-argument', `Duplicate alias or field '${alias}'`);
        }
...
function aliasedAggregateToMap(aliasedAggregatees) {
    return aliasedAggregatees.reduce((map, selectable) => {
        if (map.get(selectable.alias) !== undefined) {
            throw new common3420f234_node.FirestoreError('invalid-argument', `Duplicate alias or field '${selectable.alias}'`);
        }
...
function vectorToExpr(value) {
    if (value instanceof Expression) {
        ...
        const result = constant(common3420f234_node.vector(value));
        return result;
    }
    else {
        throw new Error('Unsupported value: ' + typeof value);
...
class Pipeline {
    ...
    removeFields(fieldValueOrOptions, ...additionalFields) {
        // Process argument union(s) from method overloads
        const options = isField(fieldValueOrOptions) || common3420f234_node.isString(fieldValueOrOptions)
            ? {}
            : fieldValueOrOptions;
        const fields = isField(fieldValueOrOptions) || common3420f234_node.isString(fieldValueOrOptions)
            ? [fieldValueOrOptions, ...additionalFields]
            : fieldValueOrOptions.fields;
        // Convert user land convenience types to internal types
        const convertedFields = fields.map(f => common3420f234_node.isString(f) ? field(f) : f);
        // Create stage object
    ...
    select(selectionOrOptions, ...additionalSelections) {
        // Process argument union(s) from method overloads
        const options = isSelectable(selectionOrOptions) || common3420f234_node.isString(selectionOrOptions)
            ? {}
            : selectionOrOptions;
        const selections = isSelectable(selectionOrOptions) || common3420f234_node.isString(selectionOrOptions)
    ...
    offset(offsetOrOptions) {
        ...
        let options;
        let offset;
        if (common3420f234_node.isNumber(offsetOrOptions)) {
            options = {};
            offset = offsetOrOptions;
        }
        else {
    ...
    limit(limitOrOptions) {
        // Process argument union(s) from method overloads
        const options = common3420f234_node.isNumber(limitOrOptions) ? {} : limitOrOptions;
        const limit = common3420f234_node.isNumber(limitOrOptions)
            ? limitOrOptions
    ...
    distinct(groupOrOptions, ...additionalGroups) {
        // Process argument union(s) from method overloads
        const options = common3420f234_node.isString(groupOrOptions) || isSelectable(groupOrOptions)
            ? {}
            : groupOrOptions;
        const groups = common3420f234_node.isString(groupOrOptions) || isSelectable(groupOrOptions)
    ...
    replaceWith(valueOrOptions) {
        // Process argument union(s) from method overloads
        const options = common3420f234_node.isString(valueOrOptions) || isExpr(valueOrOptions) ? {} : valueOrOptions;
        const fieldNameOrExpr = common3420f234_node.isString(valueOrOptions) || isExpr(valueOrOptions)
            ? valueOrOptions
            : valueOrOptions.map;
        // Convert user land convenience types to internal types
    ...
    sample(documentsOrOptions) {
        // Process argument union(s) from method overloads
        const options = common3420f234_node.isNumber(documentsOrOptions) ? {} : documentsOrOptions;
        let rate;
        let mode;
        if (common3420f234_node.isNumber(documentsOrOptions)) {
            rate = documentsOrOptions;
            mode = 'documents';
        }
        else if (common3420f234_node.isNumber(documentsOrOptions.documents)) {
    ...
    unnest(selectableOrOptions, indexField) {
        ...
        if (common3420f234_node.isString(indexFieldName)) {
...
function execute(pipeline) {
    const datastore = common3420f234_node.getDatastore(pipeline._db);
    const udr = new common3420f234_node.UserDataReader(pipeline._db._databaseId, 
    ...
    return common3420f234_node.invokeExecutePipeline(datastore, structuredPipeline).then(result => {
        ...
        const docs = result
            ...
            ? new common3420f234_node.DocumentReference(pipeline._db, null, element.key)
 ...
 * ```
 */
common3420f234_node.Firestore.prototype.pipeline = function () {
    const userDataWriter = new common3420f234_node.LiteUserDataWriter(this);
    const userDataReader = common3420f234_node.newUserDataReader(this);
