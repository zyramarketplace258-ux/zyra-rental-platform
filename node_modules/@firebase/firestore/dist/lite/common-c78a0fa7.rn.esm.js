import { FirebaseError as e, isCloudWorkstation as t, base64 as r, DecodeBase64StringError as n, getDefaultEmulatorHostnameAndPort as i, pingServer as s, updateEmulatorBanner as o, deepEqual as a, createMockUserToken as u, getModularInstance as _ } from "@firebase/util";

import { Logger as c, LogLevel as l } from "@firebase/logger";

import { Integer as h } from "@firebase/webchannel-wrapper/bloom-blob";

import { _isFirebaseServerApp as f, _getProvider, getApp as d, _removeServiceInstance as m } from "@firebase/app";
 ...
 * limitations under the License.
 */
const P = new c("@firebase/firestore");
...
function __PRIVATE_logDebug(e, ...t) {
    if (P.logLevel <= l.DEBUG) {
        const r = t.map(__PRIVATE_argToString);
        P.debug(`Firestore (${E}): ${e}`, ...r);
    }
}

function __PRIVATE_logError(e, ...t) {
    if (P.logLevel <= l.ERROR) {
        const r = t.map(__PRIVATE_argToString);
        P.error(`Firestore (${E}): ${e}`, ...r);
    }
}
 ...
 */ function __PRIVATE_logWarn(e, ...t) {
    if (P.logLevel <= l.WARN) {
        const r = t.map(__PRIVATE_argToString);
        P.warn(`Firestore (${E}): ${e}`, ...r);
    }
}
...
function __PRIVATE__fail(e, t, r) {
    ...
    let n = `FIRESTORE (${E}) INTERNAL ASSERTION FAILED: ${t} (ID: ${e.toString(16)})`;
    if (void 0 !== r) try {
...
/** An error returned by a Firestore operation. */ class FirestoreError extends e {
    /** @hideconstructor */
    constructor(
    /**
...
/** AppCheck token provider for the Lite SDK. */ class __PRIVATE_LiteAppCheckTokenProvider {
    constructor(e, t) {
        this.m = t, this.appCheck = null, this.P = null, f(e) && e.settings.appCheckToken && (this.P = e.settings.appCheckToken), 
        t.onInit((e => {
            this.appCheck = e;
        }));
    }
...
/** The default database name for a project. */ const R = "(default)";
 ...
 */ class DatabaseId {
    constructor(e, t) {
        this.projectId = e, this.database = t || R;
    }
    static empty() {
...
function __PRIVATE_databaseIdFromApp(e, t) {
    if (!Object.prototype.hasOwnProperty.apply(e.options, [ "projectId" ])) throw new FirestoreError(T.INVALID_ARGUMENT, '"projectId" not provided in firebase.initializeApp.');
 ...
 */ class BasePath {
    ...
    static extractNumericId(e) {
        return h.fromString(e.substring(4, e.length - 2));
    }
}
 ...
 */ class ResourcePath extends BasePath {
     ...
     */    static fromString(...e) {
        ...
        const t = [];
        for (const r of e) {
            if (r.indexOf("//") >= 0) throw new FirestoreError(T.INVALID_ARGUMENT, `Invalid segment (${r}). Paths must not contain // in them.`);
 ...
 */ class FieldPath$1 extends BasePath {
     ...
     */    isKeyField() {
        return 1 === this.length && this.get(0) === I;
    }
    /**
     * The field designating the key of a document.
     */    static keyField() {
        return new FieldPath$1([ I ]);
    }
    /**
     ...
     */    static fromServerFormat(e) {
        ...
        const __PRIVATE_addCurrentSegment = () => {
            if (0 === r.length) throw new FirestoreError(T.INVALID_ARGUMENT, `Invalid field path (${e}). Paths must not be empty, begin with '.', end with '.', or contain '..'`);
            t.push(r), r = "";
        };
        let i = !1;
        for (;n < e.length; ) {
            const t = e[n];
            if ("\\" === t) {
                if (n + 1 === e.length) throw new FirestoreError(T.INVALID_ARGUMENT, "Path has trailing escape character: " + e);
                const t = e[n + 1];
                if ("\\" !== t && "." !== t && "`" !== t) throw new FirestoreError(T.INVALID_ARGUMENT, "Path has invalid escape sequence: " + e);
                r += t, n += 2;
            } else "`" === t ? (i = !i, n++) : "." !== t || i ? (r += t, n++) : (__PRIVATE_addCurrentSegment(), 
            n++);
        }
        if (__PRIVATE_addCurrentSegment(), i) throw new FirestoreError(T.INVALID_ARGUMENT, "Unterminated ` in path: " + e);
 ...
 */ function __PRIVATE_validateNonEmptyArgument(e, t, r) {
    if (!r) throw new FirestoreError(T.INVALID_ARGUMENT, `Function ${e}() cannot be called with an empty ${t}.`);
}
...
function __PRIVATE_validateDocumentPath(e) {
    if (!DocumentKey.isDocumentKey(e)) throw new FirestoreError(T.INVALID_ARGUMENT, `Invalid document reference. Document references must have an even number of segments, but ${e} has ${e.length}.`);
}
 ...
 */ function __PRIVATE_validateCollectionPath(e) {
    if (DocumentKey.isDocumentKey(e)) throw new FirestoreError(T.INVALID_ARGUMENT, `Invalid collection reference. Collection references must have an odd number of segments, but ${e} has ${e.length}.`);
}
...
t) {
    ...
    e = e._delegate), !(e instanceof t)) {
        if (t.name === e.constructor.name) throw new FirestoreError(T.INVALID_ARGUMENT, "Type does not match the expected instance. Did you pass a reference from a different Firestore SDK?");
        {
            const r = __PRIVATE_valueDescription(e);
            throw new FirestoreError(T.INVALID_ARGUMENT, `Expected type '${t.name}', but it was: ${r}`);
        }
    }
    return e;
}
...
function __PRIVATE_validatePositiveNumber(e, t) {
    if (t <= 0) throw new FirestoreError(T.INVALID_ARGUMENT, `Function ${e}() requires a positive number, but it was: ${t}.`);
}
...
function __PRIVATE_generateUniqueDebugId() {
    return null === y ? y = function __PRIVATE_generateInitialUniqueDebugId() {
...
class __PRIVATE_RestConnection {
    ...
    constructor(e) {
        ...
        this.R = t + "://" + e.host, this.V = `projects/${r}/databases/${n}`, this.A = this.databaseId.database === R ? `project_id=${r}` : `project_id=${r}&database_id=${n}`;
    }
    I(e, r, n, i, s) {
        const o = __PRIVATE_generateUniqueDebugId(), a = this.p(e, r.toUriEncodedString());
        __PRIVATE_logDebug(w, `Sending RPC '${e}' ${o}:`, a, n);
        const u = {
            "google-cloud-resource-prefix": this.V,
            "x-goog-request-params": this.A
        };
        this.F(u, i, s);
        const {host: _} = new URL(a), c = t(_);
        return this.v(e, a, u, n, c).then((t => (__PRIVATE_logDebug(w, `Received RPC '${e}' ${o}: `, t), 
        t)), (t => {
            throw __PRIVATE_logWarn(w, `RPC '${e}' ${o} failed with error: `, t, "url: ", a, "request:", n), 
            t;
        }));
    }
    D(e, t, r, n, i, s) {
 ...
 */ function __PRIVATE_mapCodeFromHttpStatus(e) {
    ...
    // https://github.com/googleapis/googleapis/blob/master/google/rpc/code.proto
      ...
      default:
        return e >= 200 && e < 300 ? T.OK : e >= 400 && e < 500 ? T.FAILED_PRECONDITION : e >= 500 && e < 600 ? T.INTERNAL : T.UNKNOWN;
    }
}
 ...
 */ (v = F || (F = {}))[v.OK = 0] = "OK", v[v.CANCELLED = 1] = "CANCELLED", v[v.UNKNOWN = 2] = "UNKNOWN", 
v[v.INVALID_ARGUMENT = 3] = "INVALID_ARGUMENT", v[v.DEADLINE_EXCEEDED = 4] = "DEADLINE_EXCEEDED", 
v[v.NOT_FOUND = 5] = "NOT_FOUND", v[v.ALREADY_EXISTS = 6] = "ALREADY_EXISTS", v[v.PERMISSION_DENIED = 7] = "PERMISSION_DENIED", 
v[v.UNAUTHENTICATED = 16] = "UNAUTHENTICATED", v[v.RESOURCE_EXHAUSTED = 8] = "RESOURCE_EXHAUSTED", 
v[v.FAILED_PRECONDITION = 9] = "FAILED_PRECONDITION", v[v.ABORTED = 10] = "ABORTED", 
v[v.OUT_OF_RANGE = 11] = "OUT_OF_RANGE", v[v.UNIMPLEMENTED = 12] = "UNIMPLEMENTED", 
v[v.INTERNAL = 13] = "INTERNAL", v[v.UNAVAILABLE = 14] = "UNAVAILABLE", v[v.DATA_LOSS = 15] = "DATA_LOSS";
...
/** Converts a Base64 encoded string to a binary string. */ function __PRIVATE_decodeBase64(e) {
    try {
        ...
        r.decodeStringToByteArray(e, b));
    } catch (e) {
        throw e instanceof n ? new __PRIVATE_Base64DecodeError("Invalid base64 string: " + e) : e;
    }
}
...
const D = new RegExp(/^\d{4}-\d\d-\d\dT\d\d:\d\d:\d\d(?:\.(\d+))?Z$/);
 ...
 */ function __PRIVATE_validateJSON(e, t) {
    if (!__PRIVATE_isPlainObject(e)) throw new FirestoreError(T.INVALID_ARGUMENT, "JSON must be an object");
    let r;
    ...
    if (r) throw new FirestoreError(T.INVALID_ARGUMENT, r);
    return !0;
}
...
class Timestamp {
     ...
     */    static fromMillis(e) {
        const t = Math.floor(e / 1e3), r = Math.floor((e - 1e3 * t) * C);
     ...
     */
    t) {
        if (this.seconds = e, this.nanoseconds = t, t < 0) throw new FirestoreError(T.INVALID_ARGUMENT, "Timestamp nanoseconds out of range: " + t);
        if (t >= 1e9) throw new FirestoreError(T.INVALID_ARGUMENT, "Timestamp nanoseconds out of range: " + t);
        if (e < S) throw new FirestoreError(T.INVALID_ARGUMENT, "Timestamp seconds out of range: " + e);
        // This will break in the year 10,000.
                if (e >= 253402300800) throw new FirestoreError(T.INVALID_ARGUMENT, "Timestamp seconds out of range: " + e);
    }
    /**
     ...
     */    toMillis() {
        return 1e3 * this.seconds + this.nanoseconds / C;
    }
    _compareTo(e) {
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */ const N = "__type__", O = "__max__", q = {
    fields: {
        __type__: {
            stringValue: O
        }
    }
}, B = "__vector__", L = "value";
...
function __PRIVATE_typeOrder(e) {
    ...
    function __PRIVATE_isMaxValue(e) {
        return (((e.mapValue || {}).fields || {}).__type__ || {}).stringValue === O;
    }
    /**
 * @license
 ...
 */ (e) ? 9007199254740991 /* TypeOrder.MaxValue */ : 
    ...
    function __PRIVATE_isVectorValue(e) {
        const t = (e?.mapValue?.fields || {})[N]?.stringValue;
        return t === B;
    }
    ...
    switch (r) {
      ...
      case 10 /* TypeOrder.VectorValue */ :
        return function __PRIVATE_compareVectors(e, t) {
            const r = e.fields || {}, n = t.fields || {}, i = r[L]?.arrayValue, s = n[L]?.arrayValue, o = __PRIVATE_primitiveComparator(i?.values?.length || 0, s?.values?.length || 0);
            if (0 !== o) return o;
...
class __PRIVATE_QueryImpl {
    ...
    constructor(e, t = null, r = [], n = [], i = null, s = "F" /* LimitType.First */ , o = null, a = null) {
        ...
        this.L = null, this.startAt, this.endAt;
    }
}
 ...
 */ function __PRIVATE_queryToTarget(e) {
    const t = __PRIVATE_debugCast(e);
    return t.B || (t.B = __PRIVATE__queryToTarget(t, __PRIVATE_queryNormalizedOrderBy(e))), 
    t.B;
}
...
function fromName(e, t) {
    ...
    if (r.get(1) !== e.databaseId.projectId) throw new FirestoreError(T.INVALID_ARGUMENT, "Tried to deserialize key from different project: " + r.get(1) + " vs " + e.databaseId.projectId);
    if (r.get(3) !== e.databaseId.database) throw new FirestoreError(T.INVALID_ARGUMENT, "Tried to deserialize key from different database: " + r.get(3) + " vs " + e.databaseId.database);
 ...
 */ class __PRIVATE_DatastoreImpl extends Datastore {
    ...
    j() {
        if (this.k) throw new FirestoreError(T.FAILED_PRECONDITION, "The client has already been terminated.");
    }
    /** Invokes the provided RPC with auth and AppCheck tokens. */    I(e, t, r, n) {
        return this.j(), Promise.all([ this.authCredentials.getToken(), this.appCheckCredentials.getToken() ]).then((([i, s]) => this.connection.I(e, __PRIVATE_toResourcePath(t, r), n, i, s))).catch((e => {
            throw "FirebaseError" === e.name ? (e.code === T.UNAUTHENTICATED && (this.authCredentials.invalidateToken(), 
            this.appCheckCredentials.invalidateToken()), e) : new FirestoreError(T.UNKNOWN, e.toString());
        }));
    }
...
async function __PRIVATE_invokeRunAggregationQueryRpc(e, t, r) {
    const n = __PRIVATE_debugCast(e), {request: i, K: s, parent: o} = function __PRIVATE_toRunAggregationQueryRequest(e, t, r, n) {
        ...
        t.L = __PRIVATE__queryToTarget(t, e.explicitOrderBy)), t.L;
    }(t), r);
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */ const M = "ComponentProvider", k = new Map;
...
function __PRIVATE_getDatastore(e) {
    if (e._terminated) throw new FirestoreError(T.FAILED_PRECONDITION, "The client has already been terminated.");
    if (!k.has(e)) {
        __PRIVATE_logDebug(M, "Initializing Datastore");
 ...
 */ const x = 1048576, j = "firestore.googleapis.com", z = !0;
...
class FirestoreSettingsImpl {
    constructor(e) {
        if (void 0 === e.host) {
            if (void 0 !== e.ssl) throw new FirestoreError(T.INVALID_ARGUMENT, "Can't provide ssl option if host option is not set");
            this.host = j, this.ssl = z;
        } else this.host = e.host, this.ssl = e.ssl ?? z;
        ...
        void 0 === e.cacheSizeBytes) this.cacheSizeBytes = 41943040; else {
            if (-1 !== e.cacheSizeBytes && e.cacheSizeBytes < x) throw new FirestoreError(T.INVALID_ARGUMENT, "cacheSizeBytes must be at least 1048576");
            this.cacheSizeBytes = e.cacheSizeBytes;
        }
        !function __PRIVATE_validateIsNotUsedTogether(e, t, r, n) {
            if (!0 === t && !0 === n) throw new FirestoreError(T.INVALID_ARGUMENT, `${e} and ${r} cannot be used together.`);
        ...
        function __PRIVATE_validateLongPollingOptions(e) {
            if (void 0 !== e.timeoutSeconds) {
                if (isNaN(e.timeoutSeconds)) throw new FirestoreError(T.INVALID_ARGUMENT, `invalid long polling timeout: ${e.timeoutSeconds} (must not be NaN)`);
                if (e.timeoutSeconds < 5) throw new FirestoreError(T.INVALID_ARGUMENT, `invalid long polling timeout: ${e.timeoutSeconds} (minimum allowed value is 5)`);
                if (e.timeoutSeconds > 30) throw new FirestoreError(T.INVALID_ARGUMENT, `invalid long polling timeout: ${e.timeoutSeconds} (maximum allowed value is 30)`);
            }
        }
        /**
 * @license
...
class Firestore {
     ...
     * instance.
     */    get app() {
        if (!this._app) throw new FirestoreError(T.FAILED_PRECONDITION, "Firestore was not initialized using the Firebase SDK. 'app' is not available");
        return this._app;
    }
    get _initialized() {
    ...
    _setSettings(e) {
        if (this._settingsFrozen) throw new FirestoreError(T.FAILED_PRECONDITION, "Firestore has already been started and its settings can no longer be changed. You can only modify settings before calling any other methods on a Firestore object.");
        ...
        void 0 !== e.credentials && (this._authCredentials = function __PRIVATE_makeAuthCredentialsProvider(e) {
            ...
            switch (e.type) {
              ...
              default:
                throw new FirestoreError(T.INVALID_ARGUMENT, "makeAuthCredentialsProvider failed due to invalid credential type");
            }
        }(e.credentials));
    }
    _getSettings() {
     ...
     */    _terminate() {
        return function __PRIVATE_removeComponents(e) {
            const t = k.get(e);
            t && (__PRIVATE_logDebug(M, "Removing Datastore"), k.delete(e), t.terminate());
...
function initializeFirestore(e, t, r) {
    ...
    if (n.isInitialized(r)) throw new FirestoreError(T.FAILED_PRECONDITION, "Firestore can only be initialized once per app.");
    return n.initialize({
        options: t,
...
function getFirestore(e, t) {
    const r = "object" == typeof e ? e : d(), n = "string" == typeof e ? e : t || "(default)", s = _getProvider(r, "firestore/lite").getImmediate({
        identifier: n
    });
    if (!s._initialized) {
        const e = i("firestore");
 ...
 * Security Rules.
 */ function connectFirestoreEmulator(e, r, n, i = {}) {
    e = __PRIVATE_cast(e, Firestore);
    const _ = t(r), c = e._getSettings(), l = {
        ...c,
        emulatorOptions: e._getEmulatorOptions()
    }, h = `${r}:${n}`;
    _ && (s(`https://${h}`), o("Firestore", !0)), c.host !== j && c.host !== h && __PRIVATE_logWarn("Host has been set in both settings() and connectFirestoreEmulator(), emulator host will be used.");
    const f = {
        ...c,
        host: h,
        ssl: _,
    ...
    // enviornments which might call `connectFirestoreEmulator` multiple times as a standard practice.
        if (!a(f, l) && (e._setSettings(f), i.mockUserToken)) {
        let t, r;
        if ("string" == typeof i.mockUserToken) t = i.mockUserToken, r = User.MOCK_USER; else {
            // Let createMockUserToken validate first (catches common mistakes like
            // invalid field "uid" and missing field "sub" / "user_id".)
            t = u(i.mockUserToken, e._app?.options.projectId);
            const n = i.mockUserToken.sub || i.mockUserToken.user_id;
            if (!n) throw new FirestoreError(T.INVALID_ARGUMENT, "mockUserToken must contain 'sub' or 'user_id' field!");
            r = new User(n);
        }
 ...
 */ function terminate(e) {
    return e = __PRIVATE_cast(e, Firestore), m(e.app, "firestore/lite"), e._delete();
}
...
function collection(e, t, ...r) {
    if (e = _(e), __PRIVATE_validateNonEmptyArgument("collection", "path", t), e instanceof Firestore) {
        const n = ResourcePath.fromString(t, ...r);
        return __PRIVATE_validateCollectionPath(n), new CollectionReference(e, /* converter= */ null, n);
    }
    {
        if (!(e instanceof DocumentReference || e instanceof CollectionReference)) throw new FirestoreError(T.INVALID_ARGUMENT, "Expected first argument to collection() to be a CollectionReference, a DocumentReference or FirebaseFirestore");
 ...
 */ function collectionGroup(e, t) {
    ...
    t.indexOf("/") >= 0) throw new FirestoreError(T.INVALID_ARGUMENT, `Invalid collection ID '${t}' passed to function collectionGroup(). Collection IDs must not contain '/'.`);
    return new Query(e, 
...
function doc(e, t, ...r) {
    ...
    {
        if (!(e instanceof DocumentReference || e instanceof CollectionReference)) throw new FirestoreError(T.INVALID_ARGUMENT, "Expected first argument to doc() to be a CollectionReference, a DocumentReference or FirebaseFirestore");
        const n = e._path.child(ResourcePath.fromString(t, ...r));
 ...
 */ function refEqual(e, t) {
    return e = _(e), t = _(t), (e instanceof DocumentReference || e instanceof CollectionReference) && (t instanceof DocumentReference || t instanceof CollectionReference) && (e.firestore === t.firestore && e.path === t.path && e.converter === t.converter);
}
 ...
 */ function queryEqual(e, t) {
    return e = _(e), t = _(t), e instanceof Query && t instanceof Query && (e.firestore === t.firestore && __PRIVATE_queryEquals(e._query, t._query) && e.converter === t.converter);
}
 ...
 */ class Bytes {
     ...
     */    static fromBase64String(e) {
        try {
            ...
            throw new FirestoreError(T.INVALID_ARGUMENT, "Failed to construct data from Base64 string: " + e);
        }
    }
    /**
...
class FieldPath {
     ...
     */
    constructor(...e) {
        for (let t = 0; t < e.length; ++t) if (0 === e[t].length) throw new FirestoreError(T.INVALID_ARGUMENT, "Invalid field name at argument $(i + 1). Field names must not be empty.");
 ...
 */ class GeoPoint {
     ...
     */
    constructor(e, t) {
        if (!isFinite(e) || e < -90 || e > 90) throw new FirestoreError(T.INVALID_ARGUMENT, "Latitude must be a number between -90 and 90, but was: " + e);
        if (!isFinite(t) || t < -180 || t > 180) throw new FirestoreError(T.INVALID_ARGUMENT, "Longitude must be a number between -180 and 180, but was: " + t);
 ...
 */
     ...
     */    static fromJSON(e) {
        if (__PRIVATE_validateJSON(e, VectorValue._jsonSchema)) {
            ...
            throw new FirestoreError(T.INVALID_ARGUMENT, "Expected 'vectorValues' field to be a number array");
        }
    }
}
 ...
 */ class UserDataReader {
    ...
    /** Creates a new top-level parse context. */    createContext(e, t, r, n = !1) {
        return new ParseContextImpl({
            dataSource: e,
            methodName: t,
            targetDoc: r,
...
/** Parse document data from a set() call. */ function __PRIVATE_parseSetData(e, t, r, n, i, s = {}) {
    ...
    if (s.merge) u = new FieldMask(o.fieldMask), _ = o.fieldTransforms; else if (s.mergeFields) {
        ...
        for (const n of s.mergeFields) {
            ...
            if (!o.contains(i)) throw new FirestoreError(T.INVALID_ARGUMENT, `Field '${i}' is specified in your field mask but missing from your input data.`);
...
class __PRIVATE_ArrayUnionFieldValueImpl extends FieldValue {
    ...
    _toFieldTransform(e) {
        ...
        /*array=*/ !0), r = this.G.map((e => __PRIVATE_parseData(e, t))), n = new __PRIVATE_ArrayUnionTransformOperation(r);
        return new FieldTransform(e.path, n);
    }
    isEqual(e) {
        return e instanceof __PRIVATE_ArrayUnionFieldValueImpl && a(this.G, e.G);
    }
}
...
class __PRIVATE_ArrayRemoveFieldValueImpl extends FieldValue {
    ...
    _toFieldTransform(e) {
        const t = __PRIVATE_createSentinelChildContext(this, e, 
        /*array=*/ !0), r = this.G.map((e => __PRIVATE_parseData(e, t))), n = new __PRIVATE_ArrayRemoveTransformOperation(r);
        return new FieldTransform(e.path, n);
    }
    isEqual(e) {
        return e instanceof __PRIVATE_ArrayRemoveFieldValueImpl && a(this.G, e.G);
    }
}
...
/** Parse update data from an update() call. */ function __PRIVATE_parseUpdateData(e, t, r, n) {
    ...
    forEach(n, ((e, n) => {
        const a = __PRIVATE_fieldPathFromDotSeparatedString(t, e, r);
        // For Compat types, we have to "extract" the underlying types before
        // performing validation.
                n = _(n);
        const u = i.childContextForFieldPath(a);
        if (n instanceof __PRIVATE_DeleteFieldValueImpl) 
        // Add it to the field mask, but don't add anything to updateData.
        s.push(a); else {
            const e = __PRIVATE_parseData(n, u);
...
/** Parse update data from a list of field/value arguments. */ function __PRIVATE_parseUpdateVarargs(e, t, r, n, i, s) {
    const o = e.createContext(1 /* UserDataSource.Update */ , t, r), a = [ __PRIVATE_fieldPathFromArgument(t, n, r) ], u = [ i ];
    if (s.length % 2 != 0) throw new FirestoreError(T.INVALID_ARGUMENT, `Function ${t}() needs to be called with an even number of arguments that alternate between field names and values.`);
    for (let e = 0; e < s.length; e += 2) a.push(__PRIVATE_fieldPathFromArgument(t, s[e])), 
    u.push(s[e + 1]);
    const c = [], l = ObjectValue.empty();
    // We iterate in reverse order to pick the last value for a field if the
    // user specified the field multiple times.
    for (let e = a.length - 1; e >= 0; --e) if (!__PRIVATE_fieldMaskContains(c, a[e])) {
        const t = a[e];
        let r = u[e];
        // For Compat types, we have to "extract" the underlying types before
        // performing validation.
 ...
 */ function __PRIVATE_parseData(e, t) {
    ...
    // from firestore-exp.
    e = _(e))) return __PRIVATE_validatePlainObject("Unsupported field value:", t, e), 
 ...
 */ function __PRIVATE_fieldPathFromArgument(e, t, r) {
    ...
    // FieldPath.
    t = _(t)) instanceof FieldPath) return t._internalPath;
 ...
 */ function __PRIVATE_fieldPathFromDotSeparatedString(e, t, r) {
    if (t.search(G) >= 0) throw createError(`Invalid field path (${t}). Paths must not contain '~', '*', '/', '[', or ']'`, e, 
    /* hasConverter= */ !1, 
    /* path= */ void 0, r);
    try {
...
function createError(e, t, r, n, i) {
    ...
    u += ")"), new FirestoreError(T.INVALID_ARGUMENT, a + e + u);
}
...
function query(e, t, ...r) {
    ...
    t instanceof AppliableConstraint && n.push(t), n = n.concat(r), function __PRIVATE_validateQueryConstraintArray(e) {
        ...
        if (t > 1 || t > 0 && r > 0) throw new FirestoreError(T.INVALID_ARGUMENT, "InvalidQuery. When using composite filters, you cannot use more than one filter at the top level. Consider nesting the multiple filters within an `and(...)` statement. For example: change `query(query, where(...), or(...))` to `query(query, and(where(...), or(...)))`.");
    }
    /**
 * @license
 * Copyright 2020 Google LLC
 ...
 */ class QueryFieldFilterConstraint extends QueryConstraint {
    ...
    _parse(e) {
        const t = __PRIVATE_newUserDataReader(e.firestore), r = function __PRIVATE_newQueryFilter(e, t, r, n, i, s, o) {
            let a;
            if (i.isKeyField()) {
                if ("array-contains" /* Operator.ARRAY_CONTAINS */ === s || "array-contains-any" /* Operator.ARRAY_CONTAINS_ANY */ === s) throw new FirestoreError(T.INVALID_ARGUMENT, `Invalid Query. You can't perform '${s}' queries on documentId().`);
 ...
 */ class QueryOrderByConstraint extends QueryConstraint {
    ...
    _apply(e) {
        const t = function __PRIVATE_newQueryOrderBy(e, t, r) {
            if (null !== e.startAt) throw new FirestoreError(T.INVALID_ARGUMENT, "Invalid query. You must not call startAt() or startAfter() before calling orderBy().");
            if (null !== e.endAt) throw new FirestoreError(T.INVALID_ARGUMENT, "Invalid query. You must not call endAt() or endBefore() before calling orderBy().");
...
/** Helper function to create a bound from a document or fields */ function __PRIVATE_newQueryBoundFromDocOrFields(e, t, r, n) {
    if (r[0] = _(r[0]), r[0] instanceof DocumentSnapshot) return function __PRIVATE_newQueryBoundFromDocument(e, t, r, n, i) {
        if (!n) throw new FirestoreError(T.NOT_FOUND, `Can't use a DocumentSnapshot that doesn't exist for ${r}().`);
        const s = [];
        ...
        // results.
            ...
            if (__PRIVATE_isServerTimestamp(e)) throw new FirestoreError(T.INVALID_ARGUMENT, 'Invalid query. You are trying to start or end a query using a document for which the field "' + r.field + '" is an uncommitted server timestamp. (Since the value of this field is unknown, you cannot start/end a query with it.)');
            if (null === e) {
                const e = r.field.canonicalString();
                throw new FirestoreError(T.INVALID_ARGUMENT, `Invalid query. You are trying to start or end a query using a document for which the field '${e}' (used as the orderBy) does not exist.`);
            }
            s.push(e);
        }
 ...
 */ (e._query, e.firestore._databaseId, t, r[0]._document, n);
    {
        ...
        return function __PRIVATE_newQueryBoundFromFields(e, t, r, n, i, s) {
            ...
            if (i.length > o.length) throw new FirestoreError(T.INVALID_ARGUMENT, `Too many arguments provided to ${n}(). The number of arguments must be less than or equal to the number of orderBy() clauses`);
            const a = [];
            for (let s = 0; s < i.length; s++) {
                const u = i[s];
                if (o[s].field.isKeyField()) {
                    if ("string" != typeof u) throw new FirestoreError(T.INVALID_ARGUMENT, `Invalid query. Expected a string for document ID in ${n}(), but got a ${typeof u}`);
                    if (!__PRIVATE_isCollectionGroupQuery(e) && -1 !== u.indexOf("/")) throw new FirestoreError(T.INVALID_ARGUMENT, `Invalid query. When querying a collection and ordering by documentId(), the value passed to ${n}() must be a plain document ID, but '${u}' contains a slash.`);
                    const r = e.path.child(ResourcePath.fromString(u));
                    if (!DocumentKey.isDocumentKey(r)) throw new FirestoreError(T.INVALID_ARGUMENT, `Invalid query. When querying a collection group and ordering by documentId(), the value passed to ${n}() must result in a valid document path, but '${r}' is not because it contains an odd number of segments.`);
...
function __PRIVATE_parseDocumentIdValue(e, t, r) {
    if ("string" == typeof (r = _(r))) {
        if ("" === r) throw new FirestoreError(T.INVALID_ARGUMENT, "Invalid query. When querying with documentId(), you must provide a valid document ID, but it was an empty string.");
        if (!__PRIVATE_isCollectionGroupQuery(t) && -1 !== r.indexOf("/")) throw new FirestoreError(T.INVALID_ARGUMENT, `Invalid query. When querying a collection by documentId(), you must provide a plain document ID, but '${r}' contains a '/' character.`);
        const n = t.path.child(ResourcePath.fromString(r));
        if (!DocumentKey.isDocumentKey(n)) throw new FirestoreError(T.INVALID_ARGUMENT, `Invalid query. When querying a collection group by documentId(), the value provided must result in a valid document path, but '${n}' is not because it has an odd number of segments (${n.length}).`);
        return __PRIVATE_refValue(e, new DocumentKey(n));
    }
    if (r instanceof DocumentReference) return __PRIVATE_refValue(e, r._key);
    throw new FirestoreError(T.INVALID_ARGUMENT, `Invalid query. When querying with documentId(), you must provide a valid string or a DocumentReference, but it was: ${__PRIVATE_valueDescription(r)}.`);
}
 ...
 */ function __PRIVATE_validateDisjunctiveFilterElements(e, t) {
    if (!Array.isArray(e) || 0 === e.length) throw new FirestoreError(T.INVALID_ARGUMENT, `Invalid Query. A non-empty array is required for '${t.toString()}' filters.`);
}
 ...
 */ function __PRIVATE_validateNewFieldFilter(e, t) {
    ...
    throw r === t.op ? new FirestoreError(T.INVALID_ARGUMENT, `Invalid query. You cannot use more than one '${t.op.toString()}' filter.`) : new FirestoreError(T.INVALID_ARGUMENT, `Invalid query. You cannot use '${t.op.toString()}' filters with '${r.toString()}' filters.`);
}

function __PRIVATE_validateQueryFilterConstraint(e, t) {
    if (!(t instanceof QueryFieldFilterConstraint || t instanceof QueryCompositeFilterConstraint)) throw new FirestoreError(T.INVALID_ARGUMENT, `Function ${e}() requires AppliableConstraints created with a call to 'where(...)', 'or(...)', or 'and(...)'.`);
}
...
class AbstractUserDataWriter {
     ...
     */    convertVectorValue(e) {
        const t = e.fields?.[L].arrayValue?.values?.map((e => __PRIVATE_normalizeNumber(e.doubleValue)));
 ...
 */ function getDocs(e) {
    (function __PRIVATE_validateHasExplicitOrderByForLimitToLast(e) {
        if ("L" /* LimitType.Last */ === e.limitType && 0 === e.explicitOrderBy.length) throw new FirestoreError(T.UNIMPLEMENTED, "limitToLast() queries require specifying at least one orderBy() clause");
...
function updateDoc(e, t, r, ...n) {
    ...
    // performing validation.
        let s;
    s = "string" == typeof (t = _(t)) || t instanceof FieldPath ? __PRIVATE_parseUpdateVarargs(i, "updateDoc", e._key, t, r, n) : __PRIVATE_parseUpdateData(i, "updateDoc", e._key, t);
...
export { or as $, DocumentSnapshot as A, initializeFirestore as B, T as C, DocumentKey as D, getFirestore as E, Firestore as F, terminate as G, connectFirestoreEmulator as H, DocumentReference as I, CollectionReference as J, collection as K, collectionGroup as L, doc as M, refEqual as N, ObjectValue as O, Precondition as P, Query as Q, and as R, SnapshotVersion as S, endAt as T, endBefore as U, startAt as V, startAfter as W, limit as X, limitToLast as Y, where as Z, __PRIVATE_logDebug as _, __PRIVATE_setSDKVersion as a, orderBy as a0, query as a1, QueryConstraint as a2, QueryCompositeFilterConstraint as a3, QueryFieldFilterConstraint as a4, QueryOrderByConstraint as a5, QueryLimitConstraint as a6, QueryStartAtConstraint as a7, QueryEndAtConstraint as a8, addDoc as a9, __PRIVATE_hardAssert as aA, __PRIVATE_toStringValue as aB, __PRIVATE_toMapValue as aC, __PRIVATE_isCollectionGroupQuery as aD, __PRIVATE_isDocumentQuery as aE, __PRIVATE_queryNormalizedOrderBy as aF, FieldFilter as aG, CompositeFilter as aH, toNumber as aI, __PRIVATE_toPipelineValue as aJ, __PRIVATE_isUserData as aK, __PRIVATE_isCollectionReference as aL, __PRIVATE_isNumber as aM, UserDataReader as aN, __PRIVATE_invokeExecutePipeline as aO, deleteDoc as aa, updateDoc as ab, setDoc as ac, getDoc as ad, getDocs as ae, documentId as af, FieldValue as ag, increment as ah, arrayRemove as ai, arrayUnion as aj, serverTimestamp as ak, deleteField as al, vector as am, QueryDocumentSnapshot as an, QuerySnapshot as ao, snapshotEqual as ap, VectorValue as aq, setLogLevel as ar, Bytes as as, GeoPoint as at, Timestamp as au, __PRIVATE_isPlainObject as av, __PRIVATE_parseData as aw, FieldPath$1 as ax, __PRIVATE_isString as ay, I as az, __PRIVATE_LiteAuthCredentialsProvider as b, __PRIVATE_LiteAppCheckTokenProvider as c, __PRIVATE_databaseIdFromApp as d, __PRIVATE_cast as e, __PRIVATE_getDatastore as f, __PRIVATE_mapToArray as g, __PRIVATE_invokeRunAggregationQueryRpc as h, __PRIVATE_LiteUserDataWriter as i, __PRIVATE_fieldPathFromArgument as j, __PRIVATE_newUserDataReader as k, __PRIVATE_applyFirestoreDataConverter as l, __PRIVATE_parseSetData as m, FieldPath as n, __PRIVATE_parseUpdateVarargs as o, __PRIVATE_parseUpdateData as p, queryEqual as q, __PRIVATE_DeleteMutation as r, FirestoreError as s, __PRIVATE_invokeCommitRpc as t, __PRIVATE_invokeBatchGetDocumentsRpc as u, __PRIVATE_VerifyMutation as v, fail as w, __PRIVATE_isNullOrUndefined as x, __PRIVATE_isPermanentError as y, __PRIVATE_logError as z };
//# sourceMappingURL=common-c78a0fa7.rn.esm.js.map
